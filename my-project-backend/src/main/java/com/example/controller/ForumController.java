...
public class ForumController {
    ...
    public RestBean<WeatherVO> weather(double longitude, double latitude){
        ...
        return vo == null?
                RestBean.failure(400, "Fail to get position and weather. Please contact the administrator"
                ) : RestBean.success(vo);
    }
    ...
    public RestBean<List<TopicTypeVO>> listTypes(){
        return RestBean.success(topicService
                .listTypes()
                .stream().map(type -> type.asViewObject(TopicTypeVO.class))
                .toList());
    }
    ...
    public RestBean<TopicDetailVO> topic(@RequestParam @Min(0) int tid,
        ...
        if (topic == null) {
            return RestBean.failure(404, "Topic not found");
        }
